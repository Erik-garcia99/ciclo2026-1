...
void app_main(void)
{
    ...
    while(1){
        ...
        if(JSON != NULL){
            ESP_LOGI(TAG, "\nMAIN: %s", JSON);
            current_time = parse_time(JSON);
            ESP_LOGI(TAG, "parse time ---> %s", current_time);
            free(JSON);   // liberar memoria despues de usar
            free(current_time);
            JSON = NULL;
            current_time=NULL;
        } else {
            ESP_LOGE(TAG, "No se obtuvo respuesta HTTP");
        }
...
char *get_time(void) {
    ...
    if(err == ESP_OK){
        ...
        ESP_LOGI(TAG, "HTTP status: %d, tamanio: %d", status, content_length);
        ...
        ESP_LOGE(TAG, "Error al abrir conexion: %s", esp_err_to_name(err));
    }
